...
async def split_audio_file(
    ...
    try:
        ...
        if file_ext.lower() == ".mp3":
            print(f"Input file is an MP3 file. Skipping conversion")
            temp_mp3_path = temp_input_path
        else:
            print(f"Input file is a {file_ext} file. Converting to MP3")
...
async def extract_audio_and_split(
    ...
    try:
        ...
        print(f"Error extracting audio and splitting video file: {e}")
        raise
    finally:
        # Clean up temporary files
        shutil.rmtree(temp_dir)
