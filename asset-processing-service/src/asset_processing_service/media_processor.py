...
async def split_audio_file(
    file_buffer: bytes, max_chunk_size_bytes: int, original_file_name: str
):
    ...
    try:
        ...
        if file_ext.lower() == ".mp3":
            logger.info(f"Input file is an MP3 file. Skipping conversion")
            temp_mp3_path = temp_input_path
        else:
            logger.info(f"Input file is a {file_ext} file. Converting to MP3")
            temp_mp3_path = os.path.join(temp_dir, f"{file_name_without_ext}.mp3")
        ...
        output_pattern = os.path.join(
            temp_dir,
            f"{file_name_without_ext}_chunk_%03d.mp3",  # podcast_007_chunk_001.mp3
        )
...
async def extract_audio_and_split(
    ...
    try:
        ...
        logger.error(f"Error extracting audio and splitting video file: {e}")
        raise
    finally:
        # Clean up temporary files
        shutil.rmtree(temp_dir)
