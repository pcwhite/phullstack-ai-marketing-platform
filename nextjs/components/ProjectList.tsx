...
import { Sparkles, FileIcon, Text } from "lucide-react";
...
function ProjectList({ projects }: ProjectListProps) {
  ...
  const checkGeneratedContentStatus = async (projectId: string) => {
    try {
      const response = await fetch(
        `/api/projects/${projectId}/generated-content/status`
      );
      const data = await response.json();
  ...
  return (
    <div className="grid gap-4 sm:gap-6 md:gap-8 lg:gap-10 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
      {/* iterate through each project and convert it into its own card */}
