...
function ProjectDetailHeader({
  ...
  const handleTitleSubmit = async () => {
    try {
      setIsSaving(true);
      const res = await axios.patch<Project>(`/api/projects/${project.id}`, {
        title,
      });
      ...
      const defaultMessage = "Error updating project title. Please try again.";
  ...
  if (isEditing) {
    return (
      <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 md:space-x-2 w-full">
        <Input
          value={title}
  ...
  // TODO: Create edit description functionality
  // TODO: useState for description & editted description

  // TODO: Save description to db function
  return (
